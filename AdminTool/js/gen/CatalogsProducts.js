 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/CatalogsProducts/PopUp/ThemCatalogsProducts.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết CatalogsProducts');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvCatalogsProducts.Refresh();
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
   confirmA('Bạn có chắc chắn xóa nhóm sản phẩm này?', DoDelete);
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemCatalogsProducts() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/CatalogsProducts/PopUp/ThemCatalogsProducts.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhóm Sản Phẩm');
  ShowPopup();
  }
 function ChiTietCatalogsProducts(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/CatalogsProducts/PopUp/ThemCatalogsProducts.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Sửa Nhóm Sản Phẩm');
      ShowPopup();
     }
  }
    function ShowPopup() {
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvCatalogsProducts.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvCatalogsProducts.PerformCallback();
  }
  }
   }
