 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/NewsNotice_EmailList/PopUp/ThemNewsNotice_EmailList.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Đăng Ký Nhận Tin');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvNewsNotice_EmailList.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemNewsNotice_EmailList() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/NewsNotice_EmailList/PopUp/ThemNewsNotice_EmailList.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Đăng Ký Nhận Tin');
  ShowPopup();
  }
 function ChiTietNewsNotice_EmailList(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/NewsNotice_EmailList/PopUp/ThemNewsNotice_EmailList.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Đăng Ký Nhận Tin');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvNewsNotice_EmailList.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvNewsNotice_EmailList.PerformCallback();
  }
  }
   }
