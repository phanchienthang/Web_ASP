 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Users/PopUp/ThemUsers.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Thành Viên');
  ShowPopup();
  }
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvUsers.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn thành viên để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa thành viên này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những thành viên này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvUsers.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemUsers() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Users/PopUp/ThemUsers.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Thành Viên');
  ShowPopup();
  }
 function ChiTietUsers(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Users/PopUp/ThemUsers.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Thành Viên');
      ShowPopup();
     }
  }
