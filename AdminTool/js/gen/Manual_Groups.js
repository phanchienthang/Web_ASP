 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Manual_Groups/PopUp/ThemManual_Groups.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Nhóm Bài Viết HDSD');
  ShowPopup();
  }
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemManual_Groups() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Manual_Groups/PopUp/ThemManual_Groups.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhóm Bài Viết HDSD');
  ShowPopup();
  }
 function ChiTietManual_Groups(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Manual_Groups/PopUp/ThemManual_Groups.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Nhóm Bài Viết HDSD');
      ShowPopup();
     }
  }
 ...
 function MinMaxPopup() {
  ...
  {
     hf.Set("Width", w);
     grvManual_Groups.PerformCallback();
  }
  }
   }
