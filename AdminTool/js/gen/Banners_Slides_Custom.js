 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Banners_Slides_Custom/PopUp/ThemBanners_Slides_Custom.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Slide Ảnh Thứ 2');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvBanners_Slides_Custom.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemBanners_Slides_Custom() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Banners_Slides_Custom/PopUp/ThemBanners_Slides_Custom.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Slide Ảnh Thứ 2');
  ShowPopup();
  }
 function ChiTietBanners_Slides_Custom(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Banners_Slides_Custom/PopUp/ThemBanners_Slides_Custom.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Slide Ảnh Thứ 2');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvBanners_Slides_Custom.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvBanners_Slides_Custom.PerformCallback();
  }
  }
   }
