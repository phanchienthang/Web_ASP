 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/News_Category/PopUp/ThemNews_Category.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Nhóm Tin Tức');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvNews_Category.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemNews_Category() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/News_Category/PopUp/ThemNews_Category.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhóm Tin Tức');
  ShowPopup();
  }
 function ChiTietNews_Category(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/News_Category/PopUp/ThemNews_Category.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Nhóm Tin Tức');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  {
     hf.Set("Width", w);
     grvNews_Category.PerformCallback();
  }
  }
   }
