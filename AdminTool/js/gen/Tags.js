 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Tags/PopUp/ThemTags.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Thẻ Liên Kết');
  ShowPopup();
  }
 }
...
grvTags.Refresh();
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemTags() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Tags/PopUp/ThemTags.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Thẻ Liên Kết');
  ShowPopup();
  }
 function ChiTietTags(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Tags/PopUp/ThemTags.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Thẻ Liên Kết');
      ShowPopup();
     }
  }
