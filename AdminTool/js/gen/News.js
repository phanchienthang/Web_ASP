 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/News/PopUp/ThemNews.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Tin Tức');
  ShowPopup();
  }
 }
...
grvNews.Refresh();
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemNews() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/News/PopUp/ThemNews.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Tin Tức');
  ShowPopup();
  }
 function ChiTietNews(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/News/PopUp/ThemNews.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Tin Tức');
      ShowPopup();
     }
  }
