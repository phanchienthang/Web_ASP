 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Recruitments/PopUp/ThemRecruitments.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Tuyển Dụng');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvRecruitments.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemRecruitments() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Recruitments/PopUp/ThemRecruitments.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Tuyển Dụng');
  ShowPopup();
  }
 function ChiTietRecruitments(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Recruitments/PopUp/ThemRecruitments.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Tuyển Dụng');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  {
     hf.Set("Width", w);
     grvRecruitments.PerformCallback();
  }
  }
   }
