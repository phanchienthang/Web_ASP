 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Config/PopUp/ThemConfig.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Cấu Hình');
  ShowPopup();
  }
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvConfig.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn cấu hình để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa cấu hình này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những cấu hình này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvConfig.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemConfig() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Config/PopUp/ThemConfig.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Cấu Hình');
  ShowPopup();
  }
 function ChiTietConfig(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Config/PopUp/ThemConfig.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Cấu Hình');
      ShowPopup();
     }
  }
 ...
 function MinMaxPopup() {
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvConfig.SetHeight(h);
       grvConfig_Group.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
