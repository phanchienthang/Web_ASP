 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Solutions_Category/PopUp/ThemSolutions_Category.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Nhóm Giải Pháp');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvSolutions_Category.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemSolutions_Category() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Solutions_Category/PopUp/ThemSolutions_Category.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhóm Giải Pháp');
  ShowPopup();
  }
 function ChiTietSolutions_Category(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Solutions_Category/PopUp/ThemSolutions_Category.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Nhóm Giải Pháp');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvSolutions_Category.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvSolutions_Category.PerformCallback();
  }
  }
   }
