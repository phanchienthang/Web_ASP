 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Supports/PopUp/ThemSupports.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Nhân Viên Hỗ Trợ');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
       grvSupport_Groups.Refresh();
grvSupports.Refresh();
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvSupports.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn nhân viên hỗ trợ để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa nhân viên hỗ trợ này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những nhân viên hỗ trợ này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvSupports.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemSupports() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Supports/PopUp/ThemSupports.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhân Viên Hỗ Trợ');
  ShowPopup();
  }
 function ChiTietSupports(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Supports/PopUp/ThemSupports.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Nhân Viên Hỗ Trợ');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvSupports.SetHeight(h);
       grvSupport_Groups.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
