 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Banners_Slides/PopUp/ThemBanners_Slides.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Slide Ảnh');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvBanners_Slides.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemBanners_Slides() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Banners_Slides/PopUp/ThemBanners_Slides.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Slide Ảnh');
  ShowPopup();
  }
 function ChiTietBanners_Slides(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Banners_Slides/PopUp/ThemBanners_Slides.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Slide Ảnh');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  {
     hf.Set("Width", w);
     grvBanners_Slides.PerformCallback();
  }
  }
   }
