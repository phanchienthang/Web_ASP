 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/ModuleFunction/PopUp/ThemModuleFunction.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Danh Sách Trang Web');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
       grvModules.Refresh();
grvModuleFunction.Refresh();
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvModuleFunction.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn danh sách trang web để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa danh sách trang web này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những danh sách trang web này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvModuleFunction.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemModuleFunction() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/ModuleFunction/PopUp/ThemModuleFunction.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Danh Sách Trang Web');
  ShowPopup();
  }
 function ChiTietModuleFunction(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/ModuleFunction/PopUp/ThemModuleFunction.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Danh Sách Trang Web');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvModuleFunction.SetHeight(h);
       grvModules.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvModuleFunction.PerformCallback();
  }
  }
   }
 ...
 $('html, body').animate({
  ...
  function gotoGrid() { 
         if (window.innerWidth < 980) { 
  $('html, body').animate({
        scrollTop: $('#ModuleFunction').offset().top
   }, 100);
 }
    }
