 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/CustomThemeCssType/PopUp/ThemCustomThemeCssType.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Loại Mã Màu Đồng Bộ');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvCustomThemeCssType.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemCustomThemeCssType() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/CustomThemeCssType/PopUp/ThemCustomThemeCssType.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Loại Mã Màu Đồng Bộ');
  ShowPopup();
  }
 function ChiTietCustomThemeCssType(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/CustomThemeCssType/PopUp/ThemCustomThemeCssType.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Loại Mã Màu Đồng Bộ');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvCustomThemeCssType.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvCustomThemeCssType.PerformCallback();
  }
  }
   }
