 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Portfolios/PopUp/ThemPortfolios.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Dự Án Đã Thực Hiện');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
       grvPortfolio_Catalogs.Refresh();
grvPortfolios.Refresh();
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvPortfolios.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn dự án đã thực hiện để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa dự án đã thực hiện này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những dự án đã thực hiện này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvPortfolios.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemPortfolios() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Portfolios/PopUp/ThemPortfolios.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Dự Án Đã Thực Hiện');
  ShowPopup();
  }
 function ChiTietPortfolios(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Portfolios/PopUp/ThemPortfolios.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Dự Án Đã Thực Hiện');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvPortfolios.SetHeight(h);
       grvPortfolio_Catalogs.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
