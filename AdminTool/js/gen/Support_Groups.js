 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Support_Groups/PopUp/ThemSupport_Groups.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Nhóm Hỗ Trợ');
  ShowPopup();
  }
 }
...
grvSupport_Groups.Refresh();
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemSupport_Groups() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Support_Groups/PopUp/ThemSupport_Groups.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhóm Hỗ Trợ');
  ShowPopup();
  }
 function ChiTietSupport_Groups(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Support_Groups/PopUp/ThemSupport_Groups.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Nhóm Hỗ Trợ');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  {
     hf.Set("Width", w);
     grvSupport_Groups.PerformCallback();
  }
  }
   }
