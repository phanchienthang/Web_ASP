 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/ModuleThemes/PopUp/ThemModuleThemes.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết ModuleThemes');
  ShowPopup();
  }
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvModuleThemes.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn modulethemes để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa modulethemes này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những modulethemes này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvModuleThemes.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemModuleThemes() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/ModuleThemes/PopUp/ThemModuleThemes.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm ModuleThemes');
  ShowPopup();
  }
 function ChiTietModuleThemes(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/ModuleThemes/PopUp/ThemModuleThemes.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết ModuleThemes');
      ShowPopup();
     }
  }
 ...
 function MinMaxPopup() {
  ...
  {
     hf.Set("Width", w);
     grvModuleThemes.PerformCallback();
  }
  }
   }
 ...
 $('html, body').animate({
  ...
  function gotoGrid() { 
         if (window.innerWidth < 980) { 
  $('html, body').animate({
        scrollTop: $('#ModuleThemes').offset().top
   }, 100);
 }
    }
