 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Define_TextShow/PopUp/ThemDefine_TextShow.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Cấu Hình Hiển Thị');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvDefine_TextShow.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemDefine_TextShow() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Define_TextShow/PopUp/ThemDefine_TextShow.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Cấu Hình Hiển Thị');
  ShowPopup();
  }
 function ChiTietDefine_TextShow(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Define_TextShow/PopUp/ThemDefine_TextShow.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Cấu Hình Hiển Thị');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvDefine_TextShow.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvDefine_TextShow.PerformCallback();
  }
  }
   }
