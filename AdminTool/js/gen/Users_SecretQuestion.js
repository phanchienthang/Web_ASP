 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Users_SecretQuestion/PopUp/ThemUsers_SecretQuestion.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Câu Hỏi Bí Mật');
  ShowPopup();
  }
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemUsers_SecretQuestion() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Users_SecretQuestion/PopUp/ThemUsers_SecretQuestion.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Câu Hỏi Bí Mật');
  ShowPopup();
  }
 function ChiTietUsers_SecretQuestion(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Users_SecretQuestion/PopUp/ThemUsers_SecretQuestion.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Câu Hỏi Bí Mật');
      ShowPopup();
     }
  }
 ...
 function MinMaxPopup() {
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvUsers_SecretQuestion.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvUsers_SecretQuestion.PerformCallback();
  }
  }
   }
