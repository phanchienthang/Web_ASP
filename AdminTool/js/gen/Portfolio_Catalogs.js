 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Portfolio_Catalogs/PopUp/ThemPortfolio_Catalogs.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Nhóm Dự Án Đã Thực Hiện');
  ShowPopup();
  }
 }
...
grvPortfolio_Catalogs.Refresh();
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemPortfolio_Catalogs() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Portfolio_Catalogs/PopUp/ThemPortfolio_Catalogs.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhóm Dự Án Đã Thực Hiện');
  ShowPopup();
  }
 function ChiTietPortfolio_Catalogs(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Portfolio_Catalogs/PopUp/ThemPortfolio_Catalogs.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Nhóm Dự Án Đã Thực Hiện');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  {
     hf.Set("Width", w);
     grvPortfolio_Catalogs.PerformCallback();
  }
  }
   }
