 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Images/PopUp/ThemImages.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Images');
  ShowPopup();
  }
 }
...
grvImages.Refresh();
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvImages.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn images để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa images này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những images này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvImages.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemImages() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Images/PopUp/ThemImages.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Images');
  ShowPopup();
  }
 function ChiTietImages(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Images/PopUp/ThemImages.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Images');
      ShowPopup();
     }
  }
