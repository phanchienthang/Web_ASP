 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/LibrarySelectorCss/PopUp/ThemLibrarySelectorCss.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Thư Viện Bộ Chọn Css');
  ShowPopup();
  }
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemLibrarySelectorCss() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/LibrarySelectorCss/PopUp/ThemLibrarySelectorCss.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Thư Viện Bộ Chọn Css');
  ShowPopup();
  }
 function ChiTietLibrarySelectorCss(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/LibrarySelectorCss/PopUp/ThemLibrarySelectorCss.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Thư Viện Bộ Chọn Css');
      ShowPopup();
     }
  }
 ...
 function MinMaxPopup() {
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvLibrarySelectorCss.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvLibrarySelectorCss.PerformCallback();
  }
  }
   }
