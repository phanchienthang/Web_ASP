 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Services_Category/PopUp/ThemServices_Category.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Nhóm Dịch Vụ');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
grvServices_Category.Refresh();
 }
 function Refresh() {
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemServices_Category() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Services_Category/PopUp/ThemServices_Category.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Nhóm Dịch Vụ');
  ShowPopup();
  }
 function ChiTietServices_Category(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Services_Category/PopUp/ThemServices_Category.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Nhóm Dịch Vụ');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvServices_Category.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvServices_Category.PerformCallback();
  }
  }
   }
