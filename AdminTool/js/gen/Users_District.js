 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Users_District/PopUp/ThemUsers_District.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Quận/Huyện');
  ShowPopup();
  }
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvUsers_District.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn quận/huyện để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa quận/huyện này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những quận/huyện này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvUsers_District.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemUsers_District() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Users_District/PopUp/ThemUsers_District.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Quận/Huyện');
  ShowPopup();
  }
 function ChiTietUsers_District(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Users_District/PopUp/ThemUsers_District.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Quận/Huyện');
      ShowPopup();
     }
  }
 ...
 function MinMaxPopup() {
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvUsers_District.SetHeight(h);
       grvUsers_City.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvUsers_District.PerformCallback();
  }
  }
   }
 ...
 $('html, body').animate({
  ...
  function gotoGrid() { 
         if (window.innerWidth < 980) { 
  $('html, body').animate({
        scrollTop: $('#Users_District').offset().top
   }, 100);
 }
    }
