 ...
 function OnGridDoubleClick(values) {
    ...
    FirstLoading = true;
     p1.SetContentUrl('Module/DanhMuc/Contact_Send/PopUp/ThemContact_Send.aspx?isNew=0&double=1&id=' + values +'&ID_Form='+ID_Form);
     lblTitle.SetText('Chi Tiết Thông Tin Liên Hệ');
  ShowPopup();
  }
 }
 function setResizeLeftMenu() {
       grvContact.Refresh();
grvContact_Send.Refresh();
 }
 function Refresh() {
 ...
 function confirmDelete() {
   lp.Show();
        var i = grvContact_Send.GetSelectedRowCount();
  if (i == 0)
  {
     alertTB('Bạn vui lòng chọn thông tin liên hệ để xóa!');
     lp.Hide();
   } else
   {
      if (i == 1)
     {
         confirmA('Bạn có chắc chắn muốn xóa thông tin liên hệ này không ?', DoDelete);
     }
     else
    {
      confirmA('Bạn có chắc chắn muốn xóa những thông tin liên hệ này không?',DoDelete);
    }
   }
 }
  function DoDelete(btnVal) {
   if (btnVal == "Có") {
    ...
    if (bt) {
       ...
       grvContact_Send.UnselectAllRowsOnPage();
       lp.Hide();
  }
 }
 ...
 function XuLyThoat(btnval) {
  ...
  function ThemContact_Send() {
   FirstLoading = true;
   p1.SetContentUrl('Module/DanhMuc/Contact_Send/PopUp/ThemContact_Send.aspx?isNew=1&double=0&ID_Form='+ID_Form);
  lblTitle.SetText('Thêm Thông Tin Liên Hệ');
  ShowPopup();
  }
 function ChiTietContact_Send(values) {
    if (values != null || values != "")  {
       FirstLoading = true;
    p1.SetContentUrl('Module/DanhMuc/Contact_Send/PopUp/ThemContact_Send.aspx?isNew=0&double=0&id=' + values + '&ID_Form='+ID_Form);
    lblTitle.SetText('Chi Tiết Thông Tin Liên Hệ');
      ShowPopup();
     }
  }
 ...
 if (s.cpUpdate) { alertU(); } lp.Hide();
  ...
  function OnControlsInitialized(s, e) {
   ...
   var w = window.innerWidth;
       grvContact_Send.SetHeight(h);
       grvContact.SetHeight(h);
    if (p1.GetContentIFrameWindow().btnLuu)
    {
      var col_h = CalculateSize_Content();
      p1.GetContentIFrameWindow().setHeightFrame(col_h);
  ...
  {
     hf.Set("Width", w);
     grvContact_Send.PerformCallback();
  }
  }
   }
 ...
 $('html, body').animate({
  ...
  function gotoGrid() { 
         if (window.innerWidth < 980) { 
  $('html, body').animate({
        scrollTop: $('#Contact_Send').offset().top
   }, 100);
 }
    }
